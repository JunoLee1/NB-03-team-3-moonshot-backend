...
export default class DashboardController {
  ...
  async getKanban(req: Request, res: Response, next: NextFunction) {
    try {
      const { projectId, memberId, keyword } = req.query;

      const data = await dashboardService.getKanbanTasks(
  ...
  async getCalendar(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await dashboardService.getCalendarTasks(
