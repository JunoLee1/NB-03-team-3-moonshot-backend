...
export default class MemberService {
    // 멤버 목록 조회
    async getProjectMembers(project_id, user_id, query) {
        ...
        const membersWithTaskCount = await Promise.all(members.map(async (member) => {
    ...
    // 멤버 초대
    async inviteMember({ email, project_id, inviter_id }) {
        const project = await prisma.project.findUnique({
            where: { id: project_id },
        });
        if (!project) {
    ...
    // 초대 수락
    async acceptInvitation(member_id, user_id) {
    ...
    // 멤버 제외
    async removeMember(project_id, member_user_id, remover_id) {
        const project = await prisma.project.findUnique({
            where: { id: project_id },
        });
        if (!project) {
    ...
    // 초대 취소
    async cancelInvitation(member_id, canceller_id) {
        const member = await prisma.member.findUnique({
            where: { id: member_id },
