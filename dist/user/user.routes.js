...
router.get("/me", loginAuth, validateUser, async (req, res, next) => {
    userController.userInfoController(req, res, next);
});
// 유저 정보 수정하기 API
// 클라이언트의 정보가 존재하는지 확인 
router.patch("/me", loginAuth, validateUser, async (req, res, next) => {
    userController.userUpdateController(req, res, next);
});
// 해당 유저가 참여중인 모든 프로젝트의 할일 목록 조회 API
router.get("/me/projects", loginAuth, validateUser, async (req, res, next) => {
    userController.findUsedrProjectsController(req, res, next);
});
router.get("/me/tasks", loginAuth, validateUser, async (req, res, next) => {
    const user = req.user;
