...
export class ProjectService {
    ...
    getProjectDetails = async (userId, projectId) => {
        try {
            ...
            const isMember = projectData.members.some((member) => member.user_id === userId);
            if (!isMember) {
                throw new ForbiddenException("프로젝트에 접근할 권한이 없습니다.");
            }
            // 카운트 계산
            const memberCount = projectData.members.length;
            const taskCounts = projectData.tasks.reduce((counts, task) => {
     ...
     */
    updateProject = async (userId, projectId, projectBodyDto) => {
        try {
            ...
            // 요청한 사용자가 관리자인지 확인
            const creator = projectData.members.find((member) => member.role === "CREATOR");
            if (!creator || creator.user_id !== userId) {
                throw new ForbiddenException("프로젝트를 수정할 권한이 없습니다.");
            }
            // 모든 검증 통과 후 레포지토리에 업데이트 요청
            const updatedProjectData = await this.projectRepository.updateProject(projectId, projectBodyDto);
            // updatedProjectData 검증
            if (!updatedProjectData) {
                throw new NotFoundException("프로젝트를 업데이트 하는 중 예상치 못한 오류가 발생했습니다.");
            }
            // 수정된 최신 데이터를 최종 응답 형태에 맞도록 가공, 추후 리팩토링 시 별도의 메서드로 분리하여 사용하는게 좋을 거 같음
            const memberCount = updatedProjectData.members.length;
            const taskCounts = updatedProjectData.tasks.reduce((counts, task) => {
                if (task.taskStatus === "todo")
                    counts.todoCount++;
                else if (task.taskStatus === "inprogress")
                    counts.inProgressCount++;
